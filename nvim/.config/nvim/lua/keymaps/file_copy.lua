...
local function copy_github(is_visual)
	...
	local branch = vim.fn.system("git symbolic-ref refs/remotes/origin/HEAD"):gsub("%s+", ""):match("origin/(.+)$")
	...
	local url = ("https://github.com/%s/blob/%s/%s%s"):format(repo, branch, path, suffix)
